...
<?php 
      ...
      {
        $id = $_GET['edit_id'];
        $stmt_edit = $db_conn->prepare("SELECT * FROM slider WHERE id=:uid");
     ...
     if (isset($_POST['btn_slider_image']) && isset($_FILES['image']) && !empty($_FILES['image']['name']))  {
          ...
          $image=mt_rand(1000, 1000000).".".$imgExt;
          unlink($upload_dir.$edit_row['image']);
          move_uploaded_file($tmp_dir, $upload_dir.$image);
          $stmtImg = $db_conn->prepare("UPDATE slider SET last_time_edited_by = :editedBy, image = :myImage, timestamp = :myTime WHERE id = :uid ORDER BY   id  ASC");
  ...
  if (isset($_POST['btn_slider_content']))  {
        $title = $_POST['title'];
        $texContent = $_POST['subtitle'];
        $editedBy = $_POST['last_time_edited_by'];

    $stmt = $db_conn->prepare("UPDATE slider SET title = :myTitle, subtitle = :mySubtitle, last_time_edited_by = :editedBy, timestamp = :myTime WHERE id = :uid ORDER BY   id  ASC");
    $stmt->bindParam(':myTitle', $title);
    $stmt->bindParam(':mySubtitle', $texContent);
...
<div class="container">
      ...
      <form method="post" class="home_page_edit_form" autocomplete="off" enctype="multipart/form-data">
        <h5 class="text-center" style="color: #000;">(<em>Recommended image size is (1900px) x (950px) dimmensions)</h5><br>
        <div class="form-group">
         ...
         <div class="form-group">
  <button type="submit"   id="btn_update_item" class="btn btn-success" name="btn_slider_image" style="float: left; " title="UPDATE IMAGE"><span  class="glyphicon glyphicon-picture " ></span></button>
         </div> 
        <div class="form-group">
            <label style="float: left !important; background-color: lightgrey; border: 0px !important;" class="form-control">Edit Slider Title: </label>
          <input type="text" class="form-control" name="title" value="<?php echo $title; ?>">
         </div>
          <div class="form-group">
            <label style="float: left !important;">Edit Slider subtitle:</label>
          <textarea  class="form-control" rows="8" name="subtitle" ><?php echo $subtitle; ?></textarea>
         </div>
         <div class="form-group">
         ...
         <div class="form-group">
          ...
          <button type="submit"  id="btn_update_item" class="btn btn-success" name="btn_slider_content" style="float: left; " title="UPDATE ITEM"><span  class="glyphicon glyphicon-send " ></span></button>
